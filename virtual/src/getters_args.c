...
ssize_t			get_arguments(t_field *field, int argument, int number, t_carriage *carriage)
{
	...
	if (argument == T_REG)
	{
		parameter = get_argument(field, CUR_COORD + 1 + codage + calculate_space(main_arg, number, bytes)) - 1;
		return ((unsigned char)parameter);
	}
	else if (argument ==  GET_T_IND_ARG(T_IND))
	{
		pos = (short)get_dir(field, 1 + codage + calculate_space(main_arg, number, bytes), TWO_BYTES, carriage);
		if (COMMAND != CW_LLD)
			pos %= IDX_MOD;
		if (COMMAND == CW_ST)
			return ((short)pos);
		...
		parameter = (unsigned)get_dir(field, calculate_space(main_arg, number, bytes) + pos, FOUR_BYTES, carriage);

		return ((unsigned)parameter);
	}
	else if (argument == T_DIR)
	{
		parameter = get_dir(field, 1 + codage + calculate_space(main_arg, number, bytes), bytes, carriage);
		if (bytes == 2)
