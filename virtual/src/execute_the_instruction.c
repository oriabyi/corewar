...
{
	if (COMMAND == CW_LD || COMMAND == CW_LLD) //well
										   ...
										   t_champ *champ, t_corewar *core)
{
	...
	{
		move_carriage(field, champ->id, 1, carriage);
		return ;
	}
	argument = (get_codage(COMMAND)) ?
			get_argument(field, CUR_COORD + 1) : (unsigned char)0x80;
	if (COMMAND == CW_ZJMP)
		check_jump = jump_if_carry_instruct(field, carriage, champ->id);
	...
	{
		move_carriage(field, champ->id, (1 + fishka(argument, 3,
