...
int ft_put_error(int err, char *str)
{
	...
	(err == 5) ? ft_printf("Error: you're straddling two worlds! Chose one %s\n", str) : 0;
	(err == 6) ? ft_printf("Error: label %s has forbidden char(s)\n", str) : 0;
	(err == 7) ? ft_printf("Error: unknown operation\n", str) : 0;
	(err == 8) ? ft_printf("Error: some arguments missed or extra near %s\n",
			str) : 0;
	(err == 9) ? ft_printf("Error: type missed near %s\n", str) : 0;
...
{
	glob->commands[++(glob->cur)] = (ft_atoi(arg + 1) >> 8) & 255;
	glob->commands[++(glob->cur)] = (ft_atoi(arg + 1)) & 255;
}
